...
def prepare_features(
    ...
    if "prev_rank" not in frame.columns:
        frame["prev_rank"] = 0
    frame["prev_rank"] = pd.to_numeric(frame["prev_rank"], errors="coerce").fillna(0)

    if "rank_change" not in frame.columns and "period_rank" in frame.columns:
        frame["rank_change"] = pd.to_numeric(frame["period_rank"], errors="coerce") - frame["prev_rank"]
    elif "rank_change" not in frame.columns:
        frame["rank_change"] = 0

    for column in ["mktcap_change", "foreign_change", "turnover_change"]:
...
def apply_filters(frame: pd.DataFrame, period_end_date: pd.Timestamp | None = None) -> pd.DataFrame:
    return apply_eligibility_filter(frame, period_end_date=period_end_date)
