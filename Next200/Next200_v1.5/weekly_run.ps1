...
function Resolve-PythonExe {
    ...
    $legacyPython = "C:\Users\Admin\AppData\Local\Programs\Python\Python312\python.exe"
    if (Test-Path $legacyPython) {
        return $legacyPython
    }

    throw "Python executable was not found. Checked .venv, PATH python/py, and legacy Python312 path."
}
...
function Sync-ToLiveApp {
    ...
    try {
        ...
        Write-RunLog "Live sync skipped due to access/path issue: $TargetDir | $($_.Exception.Message)"
    }
}

Write-RunLog "Weekly automation run started"
Write-RunLog "Project root: $ProjectRoot"
Write-RunLog "Python executable: $PythonExe"
