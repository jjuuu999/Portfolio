...
@st.cache_data(show_spinner=False, ttl=300)
def load_admin_diagnostics(config: AppConfig) -> tuple[dict[str, Any], pd.DataFrame]:
    summary_path = config.auto_output_dir / "weekly_collection_summary.json"
    meta_path = config.auto_output_dir / "naver_stock_meta_weekly.csv"
    ...
    if meta_path.exists():
        try:
            meta_df = pd.read_csv(meta_path, dtype={"ticker": str})
            meta_df = normalize_ticker_columns(meta_df)
        except Exception:
            meta_df = pd.DataFrame()

    return summary_payload, meta_df
...
def build_historical_model_frame(config: AppConfig, _bundle: DataBundle, _package: dict[str, Any]) -> pd.DataFrame:
    ...
    exclude = build_eligibility_mask(history, period_end_date=history["period_end"].max())
...
def main() -> None:
    ...
    with left_col:
        ...
        admin_key = os.getenv("ADMIN_DIAGNOSTICS_KEY", "").strip()
        if admin_key:
            with st.expander("관리자 진단", expanded=False):
                entered_key = st.text_input(
                    "관리자 진단 키",
                    type="password",
                    key="admin_diagnostics_key_input",
                    help="운영 진단 정보는 관리자 키 입력 시에만 표시됩니다.",
                )
                if entered_key and entered_key == admin_key:
                    summary_payload, admin_meta_df = load_admin_diagnostics(config)
                    source_counts = pd.DataFrame(
                        [
                            {
                                "항목": "float_rate",
                                "출처 집계": json.dumps(summary_payload.get("float_rate_source_counts", {}), ensure_ascii=False),
                            },
                            {
                                "항목": "major_holder_ratio",
                                "출처 집계": json.dumps(summary_payload.get("major_holder_source_counts", {}), ensure_ascii=False),
                            },
                            {
                                "항목": "treasury_ratio",
                                "출처 집계": json.dumps(summary_payload.get("treasury_source_counts", {}), ensure_ascii=False),
                            },
                        ]
                    )
                    st.caption("이번 주 자동 수집 기준 fallback/출처 상태")
                    st.dataframe(source_counts, use_container_width=True, hide_index=True)

                    if not admin_meta_df.empty:
                        float_fallback_df = admin_meta_df.loc[
                            admin_meta_df.get("float_rate_source", pd.Series(index=admin_meta_df.index, dtype="object"))
                            .fillna("")
                            .ne("naver_actual"),
                            [column for column in ["ticker", "company", "float_rate", "float_rate_source"] if column in admin_meta_df.columns],
                        ].copy()
                        holder_fallback_df = admin_meta_df.loc[
                            (
                                admin_meta_df.get("major_holder_source", pd.Series(index=admin_meta_df.index, dtype="object")).fillna("").ne("naver_actual")
                            )
                            | (
                                admin_meta_df.get("treasury_source", pd.Series(index=admin_meta_df.index, dtype="object")).fillna("").ne("naver_actual")
                            ),
                            [
                                column
                                for column in [
                                ...
                                if column in admin_meta_df.columns
                            ],
                        ].copy()

                        metric_col1, metric_col2 = st.columns(2, gap="small")
                        with metric_col1:
                            render_metric_card("float_rate fallback", f"{len(float_fallback_df):,}개", "naver_actual 이외")
                        with metric_col2:
                            render_metric_card("holder/treasury fallback", f"{len(holder_fallback_df):,}개", "naver_actual 이외")

                        if not float_fallback_df.empty:
                            st.markdown("**float_rate fallback 종목**")
                            st.dataframe(float_fallback_df, use_container_width=True, hide_index=True)
                        if not holder_fallback_df.empty:
                            st.markdown("**major_holder / treasury fallback 종목**")
                            st.dataframe(holder_fallback_df, use_container_width=True, hide_index=True)
                    else:
                        st.info("관리자 진단용 메타 파일이 아직 없습니다.")
                elif entered_key:
                    st.error("관리자 진단 키가 올바르지 않습니다.")
